...
$.oaoTime = {
    ...
    takeCount: function () {
        ...
        for (var i = 0, j = timers.length; i < j; i++) {
            ...
            var newTimeText = "";
            //if (days > 0) {
            //    newTimeText = "<i>" + days + "</i><em>天</em>";
            //}
            hours = hours + days * 24;
            if (hours < 10) {
                newTimeText = newTimeText + "<i>0" + hours + "</i><em>:</em>";
            }
            else {
                newTimeText = newTimeText + "<i>" + hours + "</i><em>:</em>";
            }
            if (minutes < 10) {
                newTimeText = newTimeText + "<i>0" + minutes + "</i><em>:</em>";
            }
            else {
                newTimeText = newTimeText + "<i>" + minutes + "</i><em>:</em>";
            }
            if (seconds < 10) {
                newTimeText = newTimeText + "<i>0" + seconds + "</i>";
            }
            else {
                newTimeText = newTimeText + "<i>" + seconds + "</i>";
            }
